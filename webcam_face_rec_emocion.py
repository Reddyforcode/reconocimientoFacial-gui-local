...
def test(n_frames=5000, width=2688, height=1520):
    ...
    while (True):
        ...
        for (top, right, bottom, left), name, acc in zip(face_locations, face_names, face_values):
            ...
            face_landmark_list = face_recognition.face_landmarks(frame[top:bottom, left:right])
            if(len(face_landmark_list) == 0):
                print("es posible que la eprsona tenga lentes de sol")
                check = False
                msg = "normal"
            else:
                check = True
            ...
            cv2.putText(frame, msg , (left + 6, bottom - 6), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255, 255, 255), 1)

            if(check):
                image_array = cv2.resize(frame[top:bottom, left:right], (128, 128))
                ##
                ...
                with tf.Session() as sess:
                    ...
                    predictions = sess.run(softmax_tensor, {'input:0': image_array})
                    ...
                    for node_id in top_k:
                        ...
                        if(cont1 == 0):
                            cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 123), 2)
                            cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
                            cv2.putText(frame, human_string, (left + 6, bottom - 6), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255, 255, 255), 1)

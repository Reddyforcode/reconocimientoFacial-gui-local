...
def getKnowPersonsFromDB():
    know_persons = []
    try:
        conn = psycopg2.connect("dbname=registro user=reddytintayaconde password=123456")
        cur =conn.cursor()
...
def compare_faces_2(known_face_encodings, face_encoding_to_check, tolerance=0.45):
...
def test(n_frames=5000, width=2688, height=1520):
    ...
    matches = compare_faces_2(known_face_encodings, obama_face_encoding)
    ...
    while (True):
        ...
        if process_this_frame:
            ...
            for face_encoding in face_encodings:
                matches = compare_faces_2(known_face_encodings, face_encoding)
                name = "Unknown"
